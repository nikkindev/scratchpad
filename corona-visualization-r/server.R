...
wrangle_data <- function() {
  ...
  date <- colnames(cases)[ncol(cases)]
  
  # Sum over all cases
  cases <- cases %>% 
    replace(is.na(.), 0) %>% 
    mutate(cases = cases[,ncol(cases)]) %>% 
  ...
  deaths <- deaths %>% 
    replace(is.na(.), 0) %>% 
    mutate(deaths = deaths[,ncol(deaths)]) %>% 
  ...
  recovered <- recovered %>% 
    replace(is.na(.), 0) %>% 
    mutate(recovered = recovered[,ncol(recovered)]) %>% 
  ...
  # Return dataframe and date
  return(list("df" = data, "date" = date))
}
...
function(input, output, session) {
  ...
  output$date <- renderText(paste("Last Updated: ",date, sep = ""))
